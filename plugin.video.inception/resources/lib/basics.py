...
wbcsmodelsdb = os.path.join(profileDir, 'wbcsmodelsdb.db')
...
def addDownLink(name, url, mode, iconimage, desc='', stream=None, fav='add', noDownload=False, contextm=None, fanart=None, duration=None, quality=None):
    ...
    if duration:
        if addon.getSetting('duration_in_name') == 'true':
            ...
            name = name + \
                duration if six.PY3 else (name.decode(
                    'utf-8') + duration).encode('utf-8')
        else:
            secs = None
            try:
                duration = duration.upper().replace('H', ':').replace('M', ':').replace(
                    'S', '').replace(' ', '').replace('IN', '0').replace('::', ':').strip()
                if ':' in duration:
                    if duration.endswith(':'):
                        duration += '0'
                    secs = sum(int(x) * 60 ** i for i,
                               x in enumerate(reversed(duration.split(':'))))
                elif duration.isdigit():
                    secs = int(duration)
                if secs is None and len(duration) > 0:
                    xbmc.log("@@@@Cumination: Duration format error: " +
                             str(duration), xbmc.LOGERROR)
            except:
                xbmc.log("@@@@Cumination: Duration format error: " +
                         str(duration), xbmc.LOGERROR)
    width = None
    if quality:
        if addon.getSetting('quality_in_name') == 'true':
            ...
            name = name + \
                quality if six.PY3 else (name.decode(
                    'utf-8') + quality).encode('utf-8')
        else:
    ...
    if desc:
        liz.setInfo(type="Video", infoLabels={
                    "Title": name, "plot": desc, "plotoutline": desc})
    else:
    ...
    contextMenuItems.append(
        ('[COLOR hotpink]' + favtext + ' favorites[/COLOR]', 'RunPlugin(' + favorite + ')'))
    if fav == 'del':
        favorite_move_to_end = (sys.argv[0]
                                ...
                                + "&name=" + urllib_parse.quote_plus(name))
        contextMenuItems.append(
            ('[COLOR hotpink]Move favorite to Top[/COLOR]', 'RunPlugin(' + favorite_move_to_end + ')'))
        favorite_move_up = (sys.argv[0]
                            ...
                            + "&name=" + urllib_parse.quote_plus(name))
        contextMenuItems.append(
            ('[COLOR hotpink]Move favorite Up[/COLOR]', 'RunPlugin(' + favorite_move_up + ')'))
        favorite_move_down = (sys.argv[0]
                              ...
                              + "&name=" + urllib_parse.quote_plus(name))
        contextMenuItems.append(
            ('[COLOR hotpink]Move favorite Down[/COLOR]', 'RunPlugin(' + favorite_move_down + ')'))
...
def resolution(quality):
    ...
    try:
        ...
        resolutions = {'SD': (640, 480), 'HD': (1280, 720), 'FULLHD': (1920, 1080), 'FHD': (
            1920, 1080), '2K': (2560, 1440), '4K': (3840, 2160), 'UHD': (3840, 2160), '8K': (7680, 4320)}
        ...
        if len(quality) > 0 and resolution == (None, None):
            xbmc.log("@@@@Cumination: Quality format error: " +
                     str(quality), xbmc.LOGERROR)
    except:
        xbmc.log("@@@@Cumination: Quality format error: " +
                 str(quality), xbmc.LOGERROR)
    return resolution
...
def addDir(name, url, mode, iconimage=None, page=None, channel=None, section=None, keyword='', Folder=True, about=None,
           custom=False, list_avail=True, listitem_id=None, custom_list=False, contextm=None, play=False, desc=''):
    u = (sys.argv[0]
         + "?url=" + urllib_parse.quote_plus(url)
         + "&mode=" + str(mode)
         + "&page=" + str(page)
    ...
    if play:
        play_url = (sys.argv[0]
                    + "?mode=" + str('camrips.Playvid')
                    + "&url=" + "https://chaturbate.com" +
                    urllib_parse.quote_plus(url)
                    + "&img=" + urllib_parse.quote_plus(iconimage)
                    + "&name=" + urllib_parse.quote_plus(name))
        contextMenuItems.append(
            ('[COLOR hotpink]Play site[/COLOR]', 'RunPlugin(' + play_url + ')'))
    if len(keyword) >= 1:
        keyw = (sys.argv[0]
    ...
    if list_avail:
        ...
        list_url = (sys.argv[0]
                    ...
                    + "&name=" + urllib_parse.quote_plus(name))
        contextMenuItems.append(
            ('[COLOR hotpink]%s[/COLOR]' % list_item_name, 'RunPlugin(' + list_url + ')'))
    if listitem_id:
        move_listitem_url = (sys.argv[0]
                             + "?mode=" + str('favorites.move_listitem')
                             + "&listitem_id=" + str(listitem_id))
        contextMenuItems.append(
            ('[COLOR hotpink]Move item to ...[/COLOR]', 'RunPlugin(' + move_listitem_url + ')'))
        listitem_url = (sys.argv[0]
                        + "?mode=" + str('favorites.remove_listitem')
                        + "&listitem_id=" + str(listitem_id))
        contextMenuItems.append(
            ('[COLOR hotpink]Remove from list[/COLOR]', 'RunPlugin(' + listitem_url + ')'))
        ...
        movedownitem_url = (sys.argv[0]
                            ...
                            + "&listitem_id=" + str(listitem_id))
        contextMenuItems.append(
            ('[COLOR hotpink]Move item Down[/COLOR]', 'RunPlugin(' + movedownitem_url + ')'))
    ...
    if custom_list:
        ...
        movedownlist_url = (sys.argv[0]
                            ...
                            + "&rowid=" + str(url))
        contextMenuItems.append(
            ('[COLOR hotpink]Move list Down[/COLOR]', 'RunPlugin(' + movedownlist_url + ')'))
...
def searchDir(url, mode, page=None, alphabet=None):
    if not alphabet:
        addDir('[COLOR hotpink]Add Keyword[/COLOR]', url, 'utils.newSearch',
               cum_image('cum-search.png'), '', mode, Folder=False)
        addDir('[COLOR hotpink]Alphabetical[/COLOR]', url,
               'utils.alphabeticalSearch', cum_image('cum-search.png'), '', mode)
        if addon.getSetting('keywords_sorted') == 'true':
            addDir('[COLOR hotpink]Unsorted Keywords[/COLOR]', url,
                   'utils.setUnsorted', cum_image('cum-search.png'), '', mode, Folder=False)
        else:
            addDir('[COLOR hotpink]Sorted Keywords[/COLOR]', url, 'utils.setSorted',
                   cum_image('cum-search.png'), '', mode, Folder=False)
    ...
    try:
        if alphabet:
            c.execute("SELECT * FROM keywords WHERE keyword LIKE ? ORDER BY keyword ASC",
                      (alphabet.lower() + '%', ))
        else:
            if addon.getSetting('keywords_sorted') == 'true':
        ...
        for (keyword,) in c.fetchall():
            ...
            name = '[COLOR deeppink]' + \
                urllib_parse.unquote_plus(keyword) + '[/COLOR]'
            addDir(name, url, mode, cum_image(
                'cum-search.png'), page=page, keyword=keyword)
    except:
        pass
    conn.close()
    eod()
