...
class URL_Dispatcher(object):
    ...
    def register(self):
        ...
        def decorator(f):
            ...
            func_args = all_args.args[0:-len(all_args.defaults)
                                      ] if all_args.defaults else all_args.args
            func_kwargs = all_args.args[len(
                func_args):] if all_args.defaults else []
    ...
    def add_dir(self, name, url, mode, iconimage=None, page=None, channel=None, section=None, keyword='', Folder=True,
                about=None, custom=False, list_avail=True, listitem_id=None, custom_list=False, contextm=None, play=False, desc=''):
        mode = self.get_full_mode(mode)
        addDir(name, url, mode, iconimage, page, channel, section, keyword, Folder, about,
               custom, list_avail, listitem_id, custom_list, contextm, play, desc)
    ...
    def add_download_link(self, name, url, mode, iconimage, desc='', stream=None, fav='add', noDownload=False, contextm=None, fanart=None, duration=None, quality=None):
        mode = self.get_full_mode(mode)
        addDownLink(name, url, mode, iconimage, desc, stream, fav,
                    noDownload, contextm, fanart, duration, quality)
    ...
    def dispatch(cls, mode, queries):
        ...
        if mode not in cls.func_registry:
            message = 'Error: Attempt to invoke unregistered mode {}'.format(
                mode)
            raise Exception(message)
        ...
        if cls.args_registry[mode]:
            ...
            for arg in cls.args_registry[mode]:
                ...
                if arg in queries:
                    ...
                    message = 'Error: mode {} requested argument {} but it was not provided.'.format(
                        mode, arg)
                    raise Exception(message)
        ...
        if 'mode' in unused_args:
            # delete mode last in case it's used by the target function
